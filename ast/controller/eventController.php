...
if ($action == 'index'):
    ...
    if ($eventBusinessLayer->getSuccess()):
        $content = Helper::fill_datatable('event', $eventDataTable, array('Event Name', 'Event Date', 'Event invitees', 'Department Name', 'User Name'), array('event_name', 'event_date', 'event_invitees_nb', 'department_id', 'event_user_order_id'), 'event_id', array(0 => 'items'));
...
elseif ($action == 'save'):
    ...
    if (Helper::is_empty_string($query_id)):
        ...
        $success = $eventBusinessLayer->addEvent($name, $_SESSION['user_pos']);
    else:
        ...
        $success = $eventBusinessLayer->editEvent($query_id, $name, $date, $invitees, $department, $user, $_SESSION['user_pos']);
    endif;
    if ($success):
        $eventDataTable = $eventBusinessLayer->getEvents();
        if ($eventBusinessLayer->getSuccess()):
...
elseif ($action == 'delete'):
    ...
    elseif ($action == 'items'):
        include_once POS_ROOT . '/content/event/eventItemsController.php';  
endif;
?>
